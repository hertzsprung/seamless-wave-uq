...
plot "`echo $uqdata_builddir`/uq/criticalSteadyState-wellBalancedH/statistics.dat" using 1:($2-$5):($2+$5) with filledcurves lc rgbcolor '#BBBBBB' notitle, \
     "`echo $uqdata_builddir`/uq/criticalSteadyState-wellBalancedH/statistics.dat" using 1:2 lc rgbcolor 'black' lw 4 title 'Topography', \
     ...
     NaN lw 6 lc rgbcolor '#0072d3' title 'Stochastic Galerkin mean $\eta$', \
     NaN with filledcurves lw 3 lc rgbcolor '#b0d6e8' title 'Stochastic Galerkin $\sigma_\eta$', \
     "`echo $uqdata_builddir`/uq/criticalSteadyState-monteCarlo/derived-statistics.dat" using 1:2 with points lc 7 lw 2 pt 7 ps 1 title 'Monte Carlo mean $\eta$', \
     "`echo $uqdata_builddir`/uq/criticalSteadyState-wellBalancedH/derived-statistics.dat" using 1:2 lw 6 lc rgbcolor '#0072d3' notitle, \
