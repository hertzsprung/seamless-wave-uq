...
class EDFSteadyState:
    ...
    def __init__(self, mesh, solver, args):
        ...
        topography = swepc.test.EDFTopography()

        self.ic.z[:,0] = [topography.z0(x) for x in mesh.C]
        self.ic.z[:,1] = [0.5 for x in mesh.C]

        self.ic.water[:,0] = [solver.elevationToWater(EDFSteadyState.eta, z)
        ...
        self.bc.downstream_water = [EDFSteadyState.eta, 0.0]
        ...
        with open(os.path.join(args.output_dir, 'edf_exact.dat'), 'w') as file:
    ...
    def randomTopographyGenerator(self, args):
        return swepc.test.RandomEDFTopography(self.mesh, stddev=0.5)
